...
def attachment_audit(attachment):
    ...
    if len(audit_spans) > 0:
        for audit_span in audit_spans:
            ...
            user = audit_span.find_next_siblings(
                "span", attrs={"class": "report-audit-user"}
            )[0].text
            action_date = audit_span.find_next_siblings(
                "span", attrs={"class": "report-audit-time"}
            )[0].text
            audit.append({"Name": user, "Date": action_date, "Action": action})
    else:
        audit_spans = soup.find_all(
            "span", attrs={"data-i18n-token": re.compile("report-audit:.*")}
        )
        for audit_span in audit_spans:
...
def crf_in_database(database, id_study, crf_id):
    ...
    """
    crf = database.run_select_query(
        """SELECT * FROM CRF WHERE id_study = %s and crf_id = %s""",
        (id_study, crf_id),
        column_names=True,
    )
    if len(crf) == 0:
...
def get_schema(database, crf_title, crf_version):
    ...
    """
    schema = database.run_select_query(
        "SELECT * FROM CRF_Schema WHERE crf_name=%s and version=%s",
        (crf_title, crf_version),
        column_names=True,
    )
    if len(schema) == 0:
...
def get_id_crf(database, id_study, crf_id):
    ...
    and crf_id.
    """
    id_crf = database.run_select_query(
        "SELECT id FROM CRF WHERE id_study=%s AND crf_id=%s", record=(id_study, crf_id)
    )
    assert len(id_crf) == 1
    id_crf = id_crf[0][0]
...
def verify_all_spans_accounted(schema, soup):
    ...
    html_span_ids = [
        item.attrs.get("id")
        for item in soup.find_all("span", attrs={"id": re.compile(".*")})
    ]
...
def add_html(database, attachment, id_study, crf_version=1.0, additional_fields=None):
    ...
    # last signer
    signers = [
        this for this in audit if this["Action"] in ["Signed Addendum", "Signed"]
    ]
    max_date = None
    last_signed = None
    signer = None
        ...
        for this_data in all_data:
            ...
            """,
                (
                    this_data["id_crf"],
                    this_data["value"],
                    this_data["html_class"],
                    this_data["html_style"],
                    this_data["html_span_id"],
                    this_data["value"],
                    this_data["html_class"],
                    this_data["html_style"],
                ),
        ...
        for attachment in attachments:
            if attachment.filename.split(".")[-1] == "html":
                try:
                    ...
                    print(
                        f"Could not get content from attachment {attachment.filename} \
                            from study with uid: {study.study_uid}.",
                        gce,
                    )
                except Exception as exc:
                    if ignore_errors:
